...
class TestHandlerUtil(unittest.TestCase):
    ...
    def simulateSettings(self, source, dest):
        # helper function to extract settings files from prepared zip file 
        # simulating combinations of config settings and archived settings
        pass

    def test_parse_config_sp(self):
        # test 0.1 sp config syntax
        # 
        test_sp = '{"runtimeSettings": [{"handlerSettings": {"protectedSettings": null, "publicSettings": {"VolumeType": "OS", "KeyEncryptionKeyURL": "", "KekVaultResourceId": "", "KeyEncryptionAlgorithm": "RSA-OAEP", "KeyVaultURL": "https://adelpackv.vault.azure.net/", "KeyVaultResourceId": "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/adelpacrg/providers/Microsoft.KeyVault/vaults/adelpackv", "EncryptionOperation": "EnableEncryption"}, "protectedSettingsCertThumbprint": null} }]}'
        self.assertIsNotNone(self.hutil._parse_config(test_sp))

    def test_parse_config_dp_enable(self):
        # 1.1 dp config syntax 
        test_dp = '{"runtimeSettings": [{"handlerSettings": {"protectedSettings": "", "publicSettings": {"AADClientID": "5bea46af-a4ff-4122-99e6-fa51f9879ffd", "VolumeType": "DATA", "KeyEncryptionKeyURL": "https://adelpackv.vault.azure.net/keys/adelpackek/a022ed2b1eba4befb0dc9dc07bf33578", "KeyEncryptionAlgorithm": "RSA-OAEP", "KeyVaultURL": "https://adelpackv.vault.azure.net", "SequenceVersion": "eec80fc4-e0a2-434e-9007-974a150c3407", "AADClientCertThumbprint": null, "EncryptionOperation": "EnableEncryption"}, "protectedSettingsCertThumbprint": "45E4EC25EECAD03EC81F8177CEF16CD3CAF6297A"} }]}'
    ...
    #     self.assertIsNotNone(self.hutil.do_parse_context('Enable'))
    #     self.assertEqual(self.hutil.get_last_nonquery_sequence_number(), 2)
